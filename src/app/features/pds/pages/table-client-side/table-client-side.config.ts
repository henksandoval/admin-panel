...
export function getTableConfig(): AppTableConfig<EmployeeViewModel> {
  return {
    ...
    actions: [
      {icon: 'edit', label: $localize`:Table|Row action@@employee.action.edit:Edit`, color: 'primary'},
      {icon: 'delete', label: $localize`:Table|Row action@@employee.action.delete:Delete`, color: 'warn', disabled: (row) => row.status === 'active'},
    ],
    trackByKey: 'id',
    stickyHeader: true,
    maxHeight: '560px',
