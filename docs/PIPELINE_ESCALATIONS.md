...
La escalación siempre produce un artefacto (`dev-assessment.md`) y actualiza `pipeline-state.json` → `status: "escalation"` antes de detenerse.
...
### Escalaciones del Reviewer Agent

El Reviewer escala implícitamente cuando emite un veredicto `DO_NOT_MERGE`:
...
**Síntoma:** El Dev Agent no puede hacer pasar el test sin contradecir la spec.
...
4. Si es `SPEC_CONFLICT` real: agregar `<!-- STATUS: NEEDS_REVISION: {conflicto específico} -->` a `spec.md` y ejecutar `resume {issue-number}` — el PO Agent clarificará

**Resultado esperado:** Spec actualizada con criterio claro → QA Agent reescribe el test afectado → Dev retoma desde el test corregido
...
4. Si el bug está confirmado: ejecutar `resume {issue-number}` — el Coordinator invocará al QA Agent con `dev-assessment.md` como contexto
5. El QA Agent corregirá el test dentro de sus límites; si el fix implica un cambio de comportamiento, escalará al PO

**Resultado esperado:** Test corregido → Dev retoma con el test actualizado

---

### Runbook: `IMPLEMENTATION_BLOCK`

**Síntoma:** El Dev Agent no sabe cómo implementar el comportamiento requerido sin:
...
2. Ejecutar `resume {issue-number}` — el Coordinator invocará al Tech Lead Agent con `dev-assessment.md` como contexto
3. El Tech Lead intentará resolver con una aclaración de implementación
4. Si el Tech Lead no puede resolverlo: el Coordinator escala al Architect Agent para revisar el diseño
...
2. El Coordinator escribe `waiting-for-approval.md` dirigiendo al humano a clarificar el requisito específico
3. El humano edita `spec.md` con la clarificación y agrega `<!-- STATUS: APPROVED_WITH_CHANGES -->`
4. Ejecutar `resume {issue-number}` — el Coordinator invocará al PO Agent para incorporar la clarificación
5. El PO actualiza `spec.md`; el pipeline avanza desde Fase 1 (el diseño puede necesitar revisión)
...
**Síntoma:** El Dev Agent no puede clasificar el fallo con confianza.

**Pasos:**

1. Ejecutar `resume {issue-number}` — el Coordinator invocará al Reviewer Agent con `dev-assessment.md` como contexto
2. El Reviewer clasifica el fallo como `SPEC_CONFLICT`, `TEST_BUG`, `IMPLEMENTATION_BLOCK`, o `AMBIGUOUS_REQUIREMENT`
...
### Runbook: Hallazgo `BLOQUEANTE` del Reviewer

**Síntoma:** El Reviewer Agent emite `DO_NOT_MERGE` con uno o más hallazgos BLOQUEANTE.

**Pasos:**

1. Leer `review-report.md` — identificar qué regla de arquitectura fue violada y por qué es BLOQUEANTE
2. El Coordinator escribe `waiting-for-approval.md` con un resumen del hallazgo y solicita intervención humana (CP4)
3. El humano confirma si el hallazgo es válido:
   - **Válido:** confirmar con `<!-- STATUS: APPROVED -->` en `review-report.md` → pipeline retrocede a Fase 1 (Architect revisará el diseño)
   - **Falso positivo:** agregar `<!-- STATUS: NEEDS_REVISION: {motivo del falso positivo} -->` → el Reviewer es reinvocado con el contexto
...
## Hypothesis
{por qué el Dev cree que está fallando}
